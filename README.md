<p align="center">
    <img src="assets_logo.png" width="200px" alt="Han Translate Logo">
</p>

<h1 align="center">Han Translate</h1>

<p align="center">
    <b>AI Realtime Interpreter — Phiên dịch viên AI realtime, dịch mọi ngôn ngữ sang mọi ngôn ngữ</b>
</p>

<p align="center">
    <a href="https://github.com/WooCSSIN/Speech-Translate/releases/latest"><img alt="Download" src="https://img.shields.io/github/downloads/WooCSSIN/Speech-Translate/total?label=downloads"></a>
    <a href="https://github.com/WooCSSIN/Speech-Translate/stargazers"><img alt="Stars" src="https://img.shields.io/github/stars/WooCSSIN/Speech-Translate?style=social"></a>
</p>
...
| Tính năng | Mô tả |
|-----------|-------|
| 🎙️ **Realtime** | Nghe → dịch → đọc tiếng Việt trong 1-3 giây |
| 🧠 **Context-aware** | Tự nhận diện ngữ cảnh (y tế, pháp lý, kinh doanh, du lịch...) |
| 🇻🇳 **Văn hóa Việt** | Xử lý xưng hô (anh/chị/em), thành ngữ, vùng miền Bắc/Trung/Nam |
| 📴 **Offline** | Tự chuyển offline khi mất mạng, không lộ dữ liệu |
| 🔄 **Adaptive** | Học từ chỉnh sửa của bạn, càng dùng càng chính xác |
| 🌐 **34 ngôn ngữ** | Dịch từ/sang 34 ngôn ngữ với giọng đọc AI tự nhiên |
| 🔒 **Bảo mật** | API key riêng mỗi máy, không hardcode credential |

## 🚀 Cài đặt nhanh

### Cách 1: Tải file cài đặt (khuyến nghị)
Tải `HanTranslate_Setup.exe` từ [Releases](https://github.com/WooCSSIN/Speech-Translate/releases) → chạy → xong.

### Cách 2: Chạy từ source
```powershell
git clone https://github.com/WooCSSIN/Speech-Translate.git
cd Speech-Translate
py -3.12 -m venv venv
.\venv\Scripts\pip.exe install torch==2.5.1+cu121 torchaudio==2.5.1+cu121 --extra-index-url https://download.pytorch.org/whl/cu121
.\venv\Scripts\pip.exe install -r requirements-interpreter.txt
.\venv\Scripts\python.exe run_interpreter.py
```

## 📖 Hướng dẫn sử dụng

1. **Mở app** → cửa sổ Han Translate hiện ra
2. **Chọn nguồn âm**: System Audio (loa) hoặc Microphone
3. **Chọn ngôn ngữ**: Từ (nguồn) → Sang (đích)
4. **Chọn lĩnh vực** (tùy chọn): Y tế / Pháp lý / Kỹ thuật / Du lịch...
5. **Chọn vùng miền** (tùy chọn): Bắc / Trung / Nam
6. **Nhấn BẮT ĐẦU** → app tự nghe và dịch realtime
7. **Đóng cửa sổ** → app ẩn vào system tray, vẫn chạy nền

**Hotkey:** `Ctrl+Shift+I` — bật/tắt nhanh

## 🏗️ System Architecture

<p align="center">
  <img src="docs/architecture_v2.png" width="900" alt="AI Realtime Interpreter - System Architecture">
</p>

> For detailed architecture documentation, see [docs/architecture_v2.md](docs/architecture_v2.md)
...
- Customizable [subtitle window](https://github.com/WooCSSIN/Speech-Translate/raw/master/preview/13.png) for live speech to text and/or speech translation
...
- Internet connection is needed **only for translation with API & downloading models** (If you want to go fully offline, you can setup [LibreTranslate](https://github.com/LibreTranslate/LibreTranslate) on your local machine and set it up in the [app settings](https://github.com/WooCSSIN/Speech-Translate/wiki/Options#libre-translate-setting))
...
> [!IMPORTANT]  
> Please take a look at the [Requirements](#requirements) first before installing. For more information about the usage of the app, please check the [wiki](https://github.com/WooCSSIN/Speech-Translate/wiki)
...
1. Download the [latest release](https://github.com/WooCSSIN/Speech-Translate/releases/latest) (There are 2 versions, CPU and GPU/CUDA)
...
- Install with **GPU (Cuda compatible)** support:
  ...
  `pip install -U git+https://github.com/WooCSSIN/Speech-Translate.git --extra-index-url https://download.pytorch.org/whl/cu118`
...
- **CPU** only:
  ...
  `pip install -U git+https://github.com/WooCSSIN/Speech-Translate.git`
...
- If you want to **install** from a **specific branch or commit**, you can do it by adding `@branch_name` or `@commit_hash` at the end of the url. Example: `pip install -U git+https://github.com/WooCSSIN/Speech-Translate.git@dev --extra-index-url https://download.pytorch.org/whl/cu118`
...
If you prefer cloning the app directly from git/github, you can follow the guide in [development (wiki)](https://github.com/WooCSSIN/Speech-Translate/wiki/Development) or [below](#setup). Doing it this way might also provide a more stable environment.

# 📚 More Information

Check out the [wiki](https://github.com/WooCSSIN/Speech-Translate/wiki) for more information about the app, user settings, how to use it, and more.
...
> [!NOTE]  
> Check the [wiki](https://github.com/WooCSSIN/Speech-Translate/wiki) for more details
...
1. Clone the repo with its submodules by running `git clone --recurse-submodules https://github.com/WooCSSIN/Speech-Translate.git`
...
Check out my other similar project called [Screen Translate](https://github.com/WooCSSIN/Screen-Translate/) a screen translator / OCR tools made possible using tesseract.
